...
//     }
// })
const outputSprite=sprites.create(image.create(100,10))
outputSprite.setFlag(SpriteFlag.RelativeToCamera,true)
outputSprite.image.fill(0)
outputSprite.left = 0
outputSprite.top=110
controller.B.onEvent(ControllerButtonEvent.Pressed, () => {
    for (let oz = 0.01, ozs = 5.6, oza = -15; oz > 0; ozs += oza / 50, oz += ozs / 50) {
        // for (let offsetZ = 0; offsetZ <= 0; offsetZ += offsetZ_V, offsetZ_V+=offsetZ_A) {
        outputSprite.image.fill(0)
        outputSprite.image.print([oz,ozs].join(),0,0)
        rcRender.setOffsetZ(rcRender.sprSelf, oz)
        pause(20)
    }
    rcRender.setOffsetZ(rcRender.sprSelf, 0)
})
